...
let RolesGuard = class RolesGuard {
    ...
    canActivate(context) {
        try {
            const reqiueredRoles = this.reflector.getAllAndOverride(role_auth_decorator_1.ROLES_KEY, [
                context.getHandler(),
                context.getClass()
            ]);
            if (!reqiueredRoles) {
            ...
            if (bearer !== 'Bearer' || !token) {
                throw new common_1.UnauthorizedException({ message: "User not authorization" });
            }
            ...
            throw new common_1.HttpException("User not ADMIN", common_1.HttpStatus.FORBIDDEN);
        }
    }
};
